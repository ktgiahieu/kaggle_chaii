...
def teacher_loss_fn(start_logits, end_logits,
    ...
    return total_normal_loss + 3*total_student_loss
...
def train_fn(df_train, valid_data_loader, model, optimizer, device, writer, model_path, scheduler=None, df_valid=None, valid_dataset=None):  
    ...
    for epoch in range(config.EPOCHS):
        ...
        if not config.is_kaggle: #colab
            ...
            torch.save(model.state_dict(), f'./last/{model_path_filename}')
            copyfile(f'./last/{model_path_filename}', f"{'/'.join(model_path.split('/')[:-1])}/last/{model_path_filename}")
