...
def run(fold, seed):
    ...
    df_train = df_train[df_train.language=='tamil'].reset_index(drop=True)
    df_valid = dfx[dfx.kfold == fold].reset_index(drop=True)
    df_valid = df_valid[df_valid.language=='tamil'].reset_index(drop=True)
    ...
    if not os.path.isdir(f'{config.MODEL_SAVE_PATH}/last'):
        os.makedirs(f'{config.MODEL_SAVE_PATH}/last')
    if not os.path.isdir(f'last'):
        os.makedirs(f'last')
